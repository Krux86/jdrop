...
export class MyJDApi {
    ...
    async reconnect() {
        ...
        const rid = nextRid();
        // Every parameter name here is lower-case, `regaintoken` included — the
        // server matches them verbatim, so spelling it `regainToken` means it
        // sees no regain token at all, fails the reconnect, and forces a manual
        // sign-in on every expiry.
    ...
    async _withReconnect(fn) {
        try {
            ...
            if (e instanceof ApiError && e.status === 403 && this.session && this.session.regainToken) {
                ...
                // Persist before the retry, not after: the reconnect just
                // rotated the regain token and the old one is spent. If the
                // retry throws and we skipped this, storage would keep the
                // spent token and the next expiry could only be resolved by
                // signing in again.
    ...
    // --- CAPTCHA calls ---------------------------------------------------
    //
    // Real RemoteAPI namespace "captcha" (org.jdownloader.api.captcha.CaptchaAPI),
    // confirmed against JDownloader's own source, not guessed. Unlike addLinks
    // (a single object param), these take individual positional args (id, a
    // number; result/type, strings) - deviceCall only auto-JSON-encodes
    // non-string params, so string args must be JSON.stringify'd here
    // explicitly or JDownloader would receive them unquoted and malformed.

    // List pending CAPTCHA jobs for a device. Each job has { id, hoster, link,
    // type, challengeType, captchaCategory, explain, timeout, created,
    // remaining } - notably no site key (see getCaptchaRawToken).
    listCaptchaJobs(deviceId) {
        return this.deviceCall(deviceId, '/captcha/list', []);
    }

    // Fetch the "rawtoken" formatted CAPTCHA data for a job - the only way to
    // get the site key needed to render the actual widget. The exact response
    // shape is UNVERIFIED: it is not documented anywhere and the reference
    // (the old MyJDownloader MV3 extension's Rc2Service.js) only shows the
    // call being made, never the parsed result. Do not assume a shape here;
    // return the decrypted response as-is and let lib/captcha.js's (equally
    // flagged) parser interpret it once confirmed against a real account.
    getCaptchaRawToken(deviceId, id) {
        return this.deviceCall(deviceId, '/captcha/get', [JSON.stringify(id), JSON.stringify('rawtoken')]);
    }

    // Submit a solved CAPTCHA token.
    solveCaptcha(deviceId, id, result) {
        return this.deviceCall(deviceId, '/captcha/solve', [JSON.stringify(id), JSON.stringify(result)]);
    }

    // Skip a CAPTCHA job. `type` must be one of the real jd.controlling.captcha
    // .SkipRequest enum names: SINGLE, BLOCK_HOSTER, BLOCK_ALL_CAPTCHAS,
    // BLOCK_PACKAGE, REFRESH, STOP_CURRENT_ACTION, TIMEOUT. (The old extension's
    // UI used its own lowercase strings for a different, local-HTTP transport -
    // those do not apply to this cloud API call.)
    skipCaptcha(deviceId, id, type) {
        return this.deviceCall(deviceId, '/captcha/skip', [JSON.stringify(id), JSON.stringify(type)]);
    }
